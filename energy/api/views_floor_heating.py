...
def _get_home_for_request(request) -> Home | None:
    ...
    if not home_id and request.method not in ("GET", "HEAD", "OPTIONS") and hasattr(request, "data") and isinstance(request.data, dict):
...
def floor_heating_status_view(request):
    ...
    try:
        ...
        logger.exception("[FloorHeating] Fehler beim Abrufen des Fußbodenheizungs-Status: %s", e)
        return Response(
            {
                "error": str(e),
                "detail": "Fehler bei der Fußbodenheizungs-Evaluierung. Bitte prüfen, ob alle Datenbank-Migrationen angewendet wurden.",
            },
            status=500,
        )
...
def floor_heating_config_view(request):
    ...
    try:
        ...
        logger.exception("[FloorHeating] Fehler beim Speichern der Konfiguration: %s", e)
...
def floor_heating_boost_view(request):
    ...
    try:
        ...
        logger.exception("[FloorHeating] Fehler beim Starten des Vorlade-Boosts: %s", e)
        return Response({"error": str(e)}, status=500)
...
def floor_heating_toggle_view(request):
    ...
    try:
        ...
        logger.exception("[FloorHeating] Fehler beim Schalten des Relais: %s", e)
        return Response({"error": str(e)}, status=500)
