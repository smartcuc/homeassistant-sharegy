...
def system_health_status_view(request):
    ...
    # Berechtigungs-Check für interne Server-Hardware & Kapazitäts-Wächter
    is_sys_or_hems_admin = False
    if request.user and request.user.is_authenticated:
        is_sys_admin = bool(
            getattr(request.user, "is_staff", False)
            or getattr(request.user, "is_superuser", False)
            or getattr(request.user, "platform_role", "") in ("system_admin", "helpdesk")
        )
        is_hems_admin = bool(
            is_sys_admin
            or (hasattr(request.user, "memberships") and request.user.memberships.filter(role__in=["admin", "user_admin", "owner"]).exists())
        )
        is_sys_or_hems_admin = is_sys_admin or is_hems_admin

    # 11. 🖥️ Server-Hardware & Kapazitäts-Wächter (Nur für HEMS-Admins & Sharegy-Sysadmins)
    ...
    server_hardware_data = None
    if is_sys_or_hems_admin:
        server_hardware_data = {
    ...
    return Response({
        ...
        "metrics": {
            ...
            "server_hardware": server_hardware_data,
        },
    })
