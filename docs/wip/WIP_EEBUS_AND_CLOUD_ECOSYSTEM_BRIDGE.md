...
**Status:** In Konzeption / Phase 1 in Umsetzung  
**Fortschritt:** 🟡 40 %  
**Priorität:** 🔴 Hoch (Ziel: Q4 2026 / Q1 2027)  
**Lead / Modul:** `energy`, `devices`, `adapters`  
...
### Kernziele:
* **§ 14a EnWG Konformität**: Netzdienliches Dimmen der Wärmepumpe auf maximal $4{,}2\,\text{kW}$ bei Netzüberlastung.
...
## 🏗️ 2. Zweistufige Architektur

```
                            ┌──────────────────────────────────────────────┐
                            │             Sharegy EMS Optimizer            │
                            └──────────────────────┬───────────────────────┘
                                                   │
                  ┌────────────────────────────────┴───────────────────────────────┐
                  │                                                               │
                  ▼ (Phase 1: Cloud-to-Cloud Bridge)                              ▼ (Phase 2: Lokaler EEBUS SHIP Stack)
    ┌───────────────────────────┐                                   ┌───────────────────────────┐
    │  Hersteller Cloud APIs    │                                   │   Lokaler SHIP/SPINE      │
    │  • myVAILLANT API         │                                   │   Microservice / Bridge   │
    │  • ViCare Developer API   │                                   │   (mDNS Discovery,        │
    │  • Home Connect REST API  │                                   │    TLS Zertifikate)       │
    └─────────────┬─────────────┘                                   └─────────────┬─────────────┘
                  │                                                               │
                  ▼                                                               ▼
    ┌───────────────────────────┐                                   ┌───────────────────────────┐
...
| **Aktorik-Logik** | 🟢 100% | SG-Ready Relais-Steuerung mit Anti-Cycling Verdichterschutz in `energy/flow_engine.py`. | Unterstützung feingranularer Sollwertanhebung via API ($^\circ\text{C}$). |
| **Phase 1: Cloud APIs** | 🟡 30% | Auth-Token-Verwaltung in `integrations/`. | REST-Clients für *Home Connect API* und *myVAILLANT API*. |
| **Phase 2: Native EEBUS** | 🔴 10% | Netzwerk-Architektur & SHIP-Konzept skizziert. | Integration einer Python-Async SPINE/SHIP Library mit mDNS-Discovery. |
...
3. **Sprint 3**: Integration in den Dispatch Hub (`/app/control`) mit Live-Statusanzeige und Schieberegler für thermische Pufferung.
