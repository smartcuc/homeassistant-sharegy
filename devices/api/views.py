...
def device_dashboard_values(request):
    ...
    for d in devices:
        ...
        sparkline_pts = [round(float(v or 0), 2) for v in sparkline_pts if v is not None]

        if len(sparkline_pts) < 3:
            recent_1m = list(
            ...
            if len(recent_1m) >= 2:
                sparkline_pts = [round(float(v or 0), 2) for v in reversed(recent_1m)]
            else:
                recent_raw = list(
