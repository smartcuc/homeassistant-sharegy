    ...
    return (
        <div className="bg-white border border-blue-200/80 rounded-2xl shadow-xs overflow-hidden ring-1 ring-blue-100">
            ...
            <div className={`p-5 bg-gradient-to-r ${filterVendor === "sungrow" ? "from-orange-50/90 via-amber-50/40" : "from-blue-50/80 via-indigo-50/30"} to-white border-b ${filterVendor === "sungrow" ? "border-orange-200/80" : "border-blue-200/80"} flex flex-wrap items-center justify-between gap-3`}>
                ...
                {/* Top Action Button: Add / Connect */}
                {!isFormOpen && (
                    <button
                        type="button"
                        onClick={handleStartAdd}
                        className={`px-3.5 py-1.5 ${filterVendor === "sungrow" ? "bg-amber-600 hover:bg-amber-700" : "bg-blue-600 hover:bg-blue-700"} text-white text-xs font-bold rounded-xl shadow-2xs transition flex items-center gap-1.5 cursor-pointer`}
                    >
                        <span>+</span>
                        <span>
                            {filterVendor === "sungrow"
                                ? t("cloud_inverter.add_another_sungrow", "+ Weiteren Sungrow Wechselrichter hinzufügen")
                                : t("cloud_inverter.add_another_inverter", "+ Weiteren Wechselrichter koppeln")}
                        </span>
                    </button>
                )}
            </div>
            ...
            <div className="p-6 space-y-6">
                {/* 1. LISTE AKTIVER / BEREITS VERBUNDENER WECHSELRICHTER */}
                ...
                {/* 2. FORMULAR: EINSTELLUNGEN / BEARBEITEN / NEU ANLEGEN */}
                {isFormOpen && (
                    <div className="p-5 rounded-2xl bg-slate-50/80 border border-slate-200/80 space-y-6 animate-in fade-in duration-200">
                        ...
                        {filterVendor === "sungrow" ? (
                            ...
                            /* ANDERE WECHSELRICHTER: HERSTELLERAUSWAHL & ZUGANGSDATEN */
                            <>
                                <div>
