                        ...
                        <td className="py-3.5 px-5 text-right">
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <button
                              ...
                              className="px-3 py-1.5 bg-sky-600 hover:bg-sky-500 text-white text-xs font-bold rounded-lg transition flex items-center gap-1 cursor-pointer"
                            >
            ...
            {filteredHomes.map((home) => (
              <div key={home.id} className="bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 shadow-xs flex flex-col justify-between space-y-4">
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                  <Link
                    to={`/app/energy?home_id=${home.id}`}
                    className="flex-1 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-200 rounded-xl text-xs font-bold text-center border transition"
                  >
                    👁️ {t("partner.btn_view_customer_ems", "Live-Ansicht")}
                  </Link>
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className="flex-1 py-2 bg-sky-600 hover:bg-sky-500 text-white rounded-xl text-xs font-bold text-center transition cursor-pointer"
                  >
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-xl w-full p-6 space-y-6 shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="space-y-2.5">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => setShowHandoverProtocol(true)}
                    className="p-3 bg-slate-50 dark:bg-slate-800/60 hover:bg-sky-50 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 rounded-2xl text-left transition cursor-pointer"
                  >
                    <div className="text-xs font-bold text-slate-900 dark:text-white flex items-center gap-1.5">
                    ...
                    <div className="text-[10px] text-slate-400 mt-0.5">{t("partner.action_protocol_desc", "PDF Übergabenachweis")}</div>
                  </button>
                </div>
              </div>
              ...
              {/* Handover Certificate Drawer Preview */}
              {showHandoverProtocol && (
                <div className="p-4 bg-slate-50 dark:bg-slate-800/60 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="text-xs font-black text-slate-900 dark:text-white">📄 {t("partner.handover_title", "Inbetriebnahmeprotokoll & Übergabenachweis")}</h4>
                      <p className="text-[10px] text-slate-500">{t("partner.handover_sub", "Eichrechtskonforme Dokumentation für Fachbetrieb & Netzbetreiber")}</p>
                    </div>
                    <button
                      type="button"
                    ...
                    >
                      <Printer className="w-3.5 h-3.5" />
                      <span>{t("partner.handover_print", "Protokoll drucken / PDF")}</span>
                    </button>
                  </div>
                  <div className="grid grid-cols-2 gap-2 text-xs p-3 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 font-mono">
                    <div>Liegenschaft: <strong className="font-sans">{selectedAsset.name}</strong></div>
                    <div>Fachpartner: <strong className="font-sans">{data?.partner_company?.name}</strong></div>
                    <div>Datum: <strong>{new Date().toLocaleDateString("de-DE")}</strong></div>
                    <div>Status: <strong className="text-emerald-600">§ 14a EnWG abgenommen</strong></div>
                  </div>
                </div>
              )}
